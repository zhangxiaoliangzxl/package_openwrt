...
int control_recv(char *device)
{
	...
	for (;;)
	{
		...
		if (_write(fd, &bt, 1) == -1) // request sender to send file
		{
			LOG_LOG("write err!\n");
			_close(fd);
		...
		LOG_LOG("send C to request send file !\n");

		for (i = 0; i < 0x1fffff; i++) delay(0xffffffff);

		if ((len = _read(fd, rxdata + count_read, 1)) == -1)
		{
			LOG_LOG("not read date !\n");
			if (errno == EAGAIN)
			{
				if (test)
				{
	...
	while (1)
	{
		if ((len = _read(fd, rxdata + count_read, 1)) == -1)
		{
			...
			LOG_LOG("read fail! we have read %d bytes\n", count_read);
				...
				switch (rxdata[0])
				{
					case SOH:
						if (CAN_CNT)
							CAN_CNT = 0;
#ifdef DEBUG
						LOG_LOG("[SOH]128 bytes per packet!\n");
#endif
					...
					case STX:
						if (CAN_CNT)
							CAN_CNT = 0;
#ifdef DEBUG
						LOG_LOG("[STX]1024 bytes per packet!\n");
#endif
					...
					case EOT:
					{
						...
						LOG_LOG("file has been completly transfered!\n");
						if (filename_change)
						{
							filename_change = 0;
							LOG_LOG("File existed! Save it as %s \n", filename);
						}
						LOG_LOG("File length: %dbytes\n", FileLenBkup);
					...
					default:
					{
						if (CAN_CNT)
							CAN_CNT = 0;
						LOG_LOG("unvalid packet head,retry!the first data is:0x%x\n", rxdata[0]);
						PacketStartFlagRecv = 1;
						count_read = 0;
		...
		if (CAN_CNT == 2)
		{
			LOG_LOG("transmittion is canceled by user!\n");
			if (filename != NULL)
				remove(filename);
			_close(fd);
			exit(0);
		...
		{
#ifdef DEBUG
			LOG_LOG("get a packet completly!count_read:%d\n", count_read);
#endif
			PacketStartFlagRecv = 1;
			count_read = 0;
			...
			switch (head_check)
			{
				case HEAD_OK:
					...
					switch (file_check)
					{
						case FILE_OPEN_ERR:
#ifdef DEBUG
							LOG_LOG("ERROR occured when open the file!\n");
#endif
							if (first_flag)
							{
						...
						case NO_MORE_FILE:
#ifdef DEBUG
							LOG_LOG("Files have been transfered completly\n");
#endif
...
#ifdef DEBUG
					LOG_LOG("ERROR! Unsupported Format!\n");
#endif
					if (first_flag)
					{
...
int procrecdata(unsigned char *rec_buf)
{
	...
	if (first_flag && (rec_buf[1] == 0)) /*the first block (if num_blocks>256,needed first_flag)*/
	{
		...
		if (strlen(filename))
		{
			LOG_LOG("receive filename %s \n", filename);
			get_length(strlen(filename), rec_buf); /*get FielLen*/

			LOG_LOG("receive filename length %d \n", FileLenBkup);
