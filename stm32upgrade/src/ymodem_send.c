...
int control_send(char *file_name, char *device)
{
	...
	for (;;)
	{
		...
		if ((len = _read(fd, &cmd_ind, 1)) == -1)
		{
			...
			{
				// sleep(10);
				// LOG_LOG("wait for recvicer send C \n");
				continue;
			}
		...
		switch (cmd_ind)
		{
			...
			case CAN:
			{
				...
				if (CAN_CNT == 2)
				{
					LOG_LOG("the transmission is canceled by user!\n");
					_close(fd);
					return (USR_CAN);
				}
				continue;
		...
		for (; send_next;)
		{
			...
			if (!Empty_Ind)
			{
				if (file_send_end) // send "EOT" to indicate end
				{
					...
					if (EOT_NUM < 2)
					{
						LOG_LOG("file is end ,send [EOT] \n");
						if (_write(fd, &cmd_ind, 1) == -1)
						{
							LOG_LOG("EOT write err!\n");
							_close(fd);
...
int real_send(char *file_name)
{
	...
	for (i = 0; i < packet_len; i++)
	{
		...
		{
			LOG_LOG("###data write err!\n");
			return (COM_WRITE_ERR);
		}
	}
	if (No_More_File)
	{
		LOG_LOG("file send end !\n");
		return (FILE_TRANS_END);
	}
...
int Procpacket(char *send_buf, char *file_name)
{
	...
	{
		LOG_LOG("can't open file %s for read!\n", file_name);
	...
	if (Empty_Ind)
	{
		...
		LOG_LOG("send end empty package \n");
...
int get_filelen(char *file_name)
{
	...
	while ((c = fgetc(fp_send)) != EOF) FileLen++;
...
int control_upgrade(char *file_name, char *device)
{
	int i = 0;
	char cmd[128] = {0};
	char readbuf[1024] = {0};
	...
	opt.c_cc[VTIME] = 30; // read 30*100ms will return, no block
	opt.c_cflag &= ~CSTOPB;
...
#if 1
	/* send gobootloader to stm32, stm32 reboot to uboot */
	memset(cmd, 0, sizeof(cmd));
	sprintf(cmd, "%s\r\n", "reset"); // gobootloader
	ret = _write(fd, cmd, strlen(cmd));
	if (ret < 0)
	{
		LOG_LOG("write to stm32 error !\n");
	...
	LOG_LOG("send reset to stm32 !\n");
	LOG_LOG("wait for stm32 reboot to bootloader !\n");
	sleep(1);
	...
	for (;;)
	{
		...
		{
			_close(fd);
			LOG_LOG("goto bootloader mode error !\n");
			return (BOOT_NO_ACK);
		}

		memset(readbuf, 0, sizeof(readbuf));
		ret = _read(fd, readbuf, sizeof(readbuf));
		if (ret <= 0)
		{
			LOG_LOG("not receive YBOOT from stm32 !\n");
			i++;
			sleep(1);
			continue;
		}

		memcpy(lastbuf + lastbuf_len, readbuf, ret);
		lastbuf_len = lastbuf_len + ret;

		/* check stm32 boot ready ?*/
		LOG_LOG("stm32 return [%s] \n", lastbuf);

		if (strstr(lastbuf, "YBOOT") == NULL)
		{
			LOG_LOG("not receive YBOOT from stm32 !\n");
			i++;
			sleep(1);
		...
		{
			LOG_LOG("receive YBOOT from stm32 !\n");
		}

		LOG_LOG("stm32 is in boot mode !\n");
		break;
	}

#endif

	/* send ubootcmd to stm32 uboot */
	LOG_LOG("send uboot req to stm32 !\n");
	memset(cmd, 0, sizeof(cmd));
	sprintf(cmd, "%s\r", "uboot");
	ret = _write(fd, cmd, strlen(cmd));
	if (ret < 0)
	{
		LOG_LOG("write to stm32 error !\n");
	...
	for (;;)
	{
		...
		{
			_close(fd);
			LOG_LOG("bootloader long time no ack !\n");
			return (BOOT_NO_ACK);
		}
		...
		if (strstr(lastbuf, "ubootack") == NULL)
		{
			LOG_LOG("not receive uboot ack from stm32 !\n");
			i++;
			sleep(1);
			continue;
		}
		...
		{
			LOG_LOG("receive uboot ack from stm32 !\n");
		}

		LOG_LOG("stm32 boot mode is ok !\n");
		break;
	}

	/* send upgrade start to stm32 */
	memset(cmd, 0, sizeof(cmd));
	sprintf(cmd, "%s\r", "upgrade");
	ret = _write(fd, cmd, strlen(cmd));
	if (ret < 0)
	{
		LOG_LOG("write upgrade to stm32 error !\n");

		_close(fd);
		return 0;
	}
	LOG_LOG("send upgrade to stm32 !\n");
	LOG_LOG("wait stm32 erase flash !\n");
	...
	for (;;)
	{
		...
		else
		{
			...
			LOG_LOG("return [%s] \n", lastbuf);
			if (strstr(lastbuf, "Erase completed") == NULL)
			{
				sleep(1);
				continue;
			}
			...
			{
				LOG_LOG("erase flash is completed !\n");
				break;
			}
		}
	}
	...
	/* start to ymodem send file */
	LOG_LOG("send firmware to stm32 !\n");
	resault = control_send(file_name, device);

	/* restart stm32 */
#if 1
	if (resault == FILE_TRANS_END)
	{
		/* reset stm32 */
		// sleep(3);
		if ((fd = _open(device, _O_RDWR | _O_BINARY)) == -1)
		...
		for (;;)
		{
			...
			if (ret <= 0)
			{
				LOG_LOG("no data read, wait for next !\n");
				i++;

				if (i >= 3)
				{
					LOG_LOG("long time no return completed, error !\n");
					close(fd);
					return (NO_COMPLETED_ACK);
				}
			...
			else
			{
				...
				if (strstr(lastbuf, "All completed") == NULL)
				{
					j++;
					if (j >= 2)
					{
				...
				else
				{
					...
					{
						reset_ready = 1;
						LOG_LOG("ready for reset !\n");
					}
					LOG_LOG("flash check && copy is completed !\n");
					break;
				}
			}
		}
		...
		if (reset_ready == 1)
		{
			LOG_LOG("send reset to bootloader !\n");
			memset(cmd, 0, sizeof(cmd));
			sprintf(cmd, "%s\r", "reset");
			ret = _write(fd, cmd, strlen(cmd));
			if (ret < 0)
			{
				LOG_LOG("write reset to boot error !\n");
			}
		...
		else
		{
			...
			for (;;)
			{
				...
				if (ret <= 0)
				{
					...
					if (i >= 3)
					{
						LOG_LOG("long time ready for reset , error !\n");
						close(fd);
						return (NO_COMPLETED_ACK);
					}
