	...
	if(PROTO_DHCP68 == ntohs(port)){
		...
		if(dhcpInfo->msgType == DHCP_BOOT_REPLY){
			...
			if(DHCP_MSG_TYPE == *pDhcpOption && DHCP_MSG_TYPE_ACK == *(pDhcpOption+2)){
				...
				/*< 解析option43，用户AP上线*/
//				printk("get dhcp mac = "MACSTR", ip=%s\n", MAC2STR(dhcpInfo->clientMac), buf);
				if(memcmp(APMac, dhcpInfo->clientMac, 6) == 0 && !strcmp(skb->dev->name, "eth0")){
...
//							printk("parse option 43\n");
							if(0 != g_cloud_wlan_nlmsg_pid){
								cloud_wlan_sendto_umod(CW_NLMSG_RECORD_DHCP_OPTION_43, pDhcpOption+2, sizeof(option43_cfg));
								g_option43_flag = 1;
							}
							else{
								memset(&g_option43_info, 0, sizeof(option43_cfg));
								memcpy(&g_option43_info, pDhcpOption+2, sizeof(option43_cfg));
							}
...
//	iphdr = ip_hdr(skb);
	...
	u16 wltpHeadProc = 0x3838;
	u16 wltpHeadType = 0x000b;
	u16 wltpHeadPort = 7070;
	unsigned int vlanId_length = 4;
#endif
...
//		printk("wltp_ret is %d#\n", wltp_ret);
		...
		skb->data = skb->head + (unsigned int)(skb->mac_header + 14);
...
void WltpSendPkt(char *pkt_data, uint16_t pkt_leng, unsigned int mark)
{
	...
	vapIndex = (mark>>1) & 0x1f;
	vapIndex = vapIndex-SSID_LIMIT_BANDWIDTH_CLASSID_OFFSET;
...
//	printk("vapIndex=%d, cardIndex=%d\n", vapIndex, cardIndex);
	...
	if (pkt_leng <= MAX_LENGTH_FOR_MAC_WLTP)
	{
		...
		LoadWltpHeader(wltp_1pkt_data, seq, pkt_leng, 0, vapIndex, cardIndex);
		memcpy(&wltp_1pkt_data[WLTP_HEADER_LEN], pkt_data, pkt_leng);
		send_udp(wltp_1pkt_data, pkt_leng + WLTP_HEADER_LEN, mark);
	}
	else
	{
		uint32_t first_seq;
		...
		LoadWltpHeader(wltp_1pkt_data, first_seq, pkt_leng, FIRST_FRAGMENT_FLAG, vapIndex, cardIndex);
		memcpy(&wltp_1pkt_data[WLTP_HEADER_LEN], pkt_data, MAX_FRAGMENT_LENGTH);
		send_udp(wltp_1pkt_data, MAX_FRAGMENT_LENGTH + WLTP_HEADER_LEN, mark);

		LoadWltpHeader(wltp_1pkt_data, second_seq, pkt_leng - MAX_FRAGMENT_LENGTH, SECOND_FRAGMENT_FLAG, vapIndex, cardIndex);
		memcpy(&wltp_1pkt_data[WLTP_HEADER_LEN], &pkt_data[MAX_FRAGMENT_LENGTH], pkt_leng - MAX_FRAGMENT_LENGTH);
		send_udp(wltp_1pkt_data, pkt_leng - MAX_FRAGMENT_LENGTH + WLTP_HEADER_LEN, mark);
	}
}
...
//	printk("****skb->mark=%x, out->name=%s\n", skb->mark, skb->dev->name);
	if(skb->mark >= 1000 && (!memcmp(skb->dev->name, "eth0", 4)))
	{
...
//		printk("****gTemplateID_1=%d, *gTemplateID_2=%d,out->name=%s, mark=%d\n", gTemplateID_1, gTemplateID_2, out->name, mark);
		...
		vlanId = htonl(0x81000000 + (mark-1000));
//		printk("****gTemplateID=%d, out->name=%s, mark=%d\n", gTemplateID_2, out->name, mark);
			
		memmove(skb_mac_header(skb)-vlanId_length, skb_mac_header(skb), 12);
		skb->mac_header -= vlanId_length;
		memcpy(skb_mac_header(skb)+12, (unsigned char *)&vlanId, 4);

		WltpSendPkt((skb->head + (unsigned int)skb->mac_header), (skb->len + 18), skb->mark);
//		skb->mark = 0;
//		cloud_wlan_sendto_umod(CW_NLMSG_UP_DTA_PACKAGE, skb->head+skb->mac_header, skb->len+18);
...
u32 kmod_hook_init(void)
{
	...
	g_cwlan_out_hook_prer.priority = NF_BR_PRI_LAST;   /* NF_BR_PRI_FIRST让我们的函数首先执行 */
