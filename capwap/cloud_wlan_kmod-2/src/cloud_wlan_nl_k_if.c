...
{  
    ...
    nlh = nlmsg_hdr(skb); 
#if 0
	if(g_cloud_wlan_nlmsg_pid ? (nlh->nlmsg_pid != g_cloud_wlan_nlmsg_pid) : 0 && g_cloud_wlan_nlmsg_pid_2 ? (nlh->nlmsg_pid != g_cloud_wlan_nlmsg_pid_2) : 0){
		if(g_cloud_wlan_nlmsg_pid && nlh->nlmsg_pid != g_cloud_wlan_nlmsg_pid){
			return;
		}
	}
#endif
...
//			printk("g_cloud_wlan_nlmsg_pid = %d\n", g_cloud_wlan_nlmsg_pid);
			if(!g_option43_flag){
