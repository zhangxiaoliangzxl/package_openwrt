...
CWBool CWAssembleTransportHeader(CWProtocolMessage *transportHdrPtr, CWProtocolTransportHeaderValues *valuesPtr) {
	...
	CWSetField32(val,
		     ...
		     (valuesPtr->radioIndex>-1)?valuesPtr->radioIndex:0); // only one radio per WTP?
...
#endif	
	// create message
	CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, (sizeof(unsigned short)*2)+(sizeof(unsigned int)*1)+200, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	
	CWProtocolStore32(msgPtr, code);
//	CWDebugLog("Result Code: %d", code);

	if(payload->getWtpVersion)
	{	
		CW_CREATE_OBJECT_ERR(msgElems, CWProtocolMessage, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
		if(!CWAssembleMsgElemWTPVendorSpecificPayloadWTPVersion(msgElems, 200))
		{
			CW_FREE_OBJECT(msgElems);
			return CW_FALSE;
		}
		CWProtocolStoreMessage(msgPtr, msgElems);
		CW_FREE_OBJECT(msgElems);
...
CWBool CWAssembleMessage(CWProtocolMessage **completeMsgPtr, int *fragmentsNumPtr, int PMTU, int seqNum, int msgTypeValue, CWProtocolMessage *msgElems, const int msgElemNum, CWProtocolMessage *msgElemsBinding, const int msgElemBindingNum, int radioIndex, int is_crypted) {
	...
	transportVal.radioIndex = radioIndex;
...
//		transportVal.last = 1;
		...
		CWLog("%d Fragments", *fragmentsNumPtr);
...
CWBool CWAssembleUnrecognizedMessageResponse(CWProtocolMessage **messagesPtr, int *fragmentsNumPtr, int PMTU, int seqNum, int msgType)
{
	...
	if(!(CWAssembleMessage(messagesPtr, fragmentsNumPtr, PMTU, seqNum, msgType, msgElems, msgElemCount, msgElemsBinding, msgElemBindingCount, -1, CW_PACKET_CRYPT))) 
		return CW_FALSE;
...
{
	if(msgPtr == NULL) return CWErrorRaise(CW_ERROR_WRONG_ARG, NULL);
	unsigned short VENDOR_ID_LENGTH  = 4;
	unsigned short ELEMENT_ID = 2;
	unsigned short ELEMENT_LENGTH = 2;
	unsigned short REPORT_TIME_LENGTH = 4;
	unsigned short STA_COUNT_LENGTH = 4;
	
	int i;
	CWListElement *current = NULL;;
//	CWListElement *tmp = NULL;
	...
	for (i=0; i<msgElemCount; i++) {
		size += ((CWMsgElemWTPEvent *) current->data)->length;
		current = current->next;
	}
	size += msgElemCount*4;
//	CWLog("elem size if %d", size);
	// create message
	if(elemType == CW_MSG_WTP_EVENT_PRIVATE_INFO)
	{
		CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, size + VENDOR_ID_LENGTH + ELEMENT_ID + ELEMENT_LENGTH + REPORT_TIME_LENGTH, 
			return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	}
	else
	{
		CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, size + VENDOR_ID_LENGTH + ELEMENT_ID + ELEMENT_LENGTH, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	}
	CWProtocolStore32(msgPtr, CW_VENDOR_ID); // z-com
	CWProtocolStore16(msgPtr, elemType);
	if(elemType == CW_MSG_WTP_EVENT_PRIVATE_INFO)
	{
		CWProtocolStore16(msgPtr, size + REPORT_TIME_LENGTH);
	}
	else
	{
		CWProtocolStore16(msgPtr, size);
	}
	if(elemType == CW_MSG_WTP_EVENT_PRIVATE_INFO)
	{
		CWProtocolStore32(msgPtr, time(NULL));
	}
#if 0
	CWLog("elem size if %d********111111111111*********", msgPtr->offset);

	CWProtocolStore16(msgPtr, 1);
	CWProtocolStore16(msgPtr, 6);
	unsigned char mac[6] = {0};
	getWTPMac(mac);
	CWProtocolStoreRawBytes(msgPtr, mac, 6);
	CWLog("elem size if %d*********22222222222********", msgPtr->offset);
	CWProtocolStore16(msgPtr, 2);
	CWProtocolStore16(msgPtr, 4);
	mac[0] = 0xc0, mac[1] = 0xa8; mac[2] = 0x01; mac[3] = 0x28;
	CWProtocolStoreRawBytes(msgPtr, mac, 4);
	CWLog("elem size if %d*********333333333333********", msgPtr->offset);
	CWProtocolStore16(msgPtr, 3);
	CWProtocolStore16(msgPtr, 65);
	char *name;
	CW_CREATE_OBJECT_SIZE_ERR(name, 65, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	CW_ZERO_MEMORY(name, 65);
	CW_COPY_MEMORY(name, CWWTPGetName(), strlen(CWWTPGetName()));
	CWProtocolStoreRawBytes(msgPtr, name, 65);

	CWProtocolStore16(msgPtr, 4);
	CWProtocolStore16(msgPtr, 1);
	CWProtocolStore8(msgPtr, 156);

	CWLog("elem size if %d*****************", msgPtr->offset);

#else
	current = msgElemList;
	for (i=0; i < msgElemCount; i++) {
		CWProtocolStore16(msgPtr, ((CWMsgElemWTPEvent *) current->data)->type);
//		CWLog("elem type if %d", ((CWMsgElemWTPEvent *) current->data)->type);
		CWProtocolStore16(msgPtr, ((CWMsgElemWTPEvent *) current->data)->length);
//		CWLog("elem length is %d", ((CWMsgElemWTPEvent *) current->data)->length);
		CWProtocolStoreRawBytes(msgPtr, ((CWMsgElemWTPEvent *) current->data)->value, ((CWMsgElemWTPEvent *) current->data)->length);

//		CWLog("elem value store ************");

//		tmp = current;
		current = current->next;
//		CWLog("elem length is %d, free tmp:%p, data: %p, value:%p", ((CWMsgElemWTPEvent *) tmp->data)->length, tmp, tmp->data, ((CWMsgElemWTPEvent *) tmp->data)->value);
//		CWLog("elem free ************");
	}
#endif
