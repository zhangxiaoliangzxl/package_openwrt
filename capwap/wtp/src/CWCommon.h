...
#define		CW_COPY_MEMORY(dst, src, len)	memcpy(dst, src, len)//	bcopy(src, dst, len)
...
typedef struct _CWDTTAllPublicInfo{
	unsigned char cardnum;
	unsigned int maxtxpower[2];
	char apModel[64];	//AP的型号
	char ethMac[64];	//AP的有线MAC
	char wlan0Mac[64];	//AP的wlan0 MAC
	char wlan1Mac[64];	//AP的wlan1 MAC	单卡时，此mac无效
	char sn[64];
	char hwModel[48];	//硬件版本号
	char fwModel[48];	//软件版本号
	char hideFwMode[48];	//隐藏版本号
	char ethIP[32];
	time_t onlinetoACtime;	//在AC上上线的时间
	char *logfile;
}CWDTTAllPublicInfo;
...
#define	CW_CREATE_ARRAY_ERR(ar_name, ar_size, ar_type, on_err)	{ar_name = (ar_type*) (malloc(sizeof(ar_type) * (ar_size))); if(!(ar_name)) {on_err} else {CW_ZERO_MEMORY(ar_name, sizeof(ar_type) * (ar_size));}}
#define	CW_CREATE_STRING_ERR(str_name, str_length, on_err)	{str_name = (char*) (malloc(sizeof(char) * ((str_length)+1) ) ); if(!(str_name)) {on_err} else {CW_ZERO_MEMORY(str_name, (str_length)+1);}}
