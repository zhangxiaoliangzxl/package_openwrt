...
/*< 此结构成员，必须是协议里面都在使用，且已经上报的，若有未使用成员，会有问题*/
...
typedef struct {
	int radioIndex;	/*< 单隧道，capwap协议头中的radio序号*/
	int payloadType;
	int type;
	int isFragment;
	int last;
...
{
    uint                funcID;                         /**< ID号 */
    CWBool              (*handler)(CWProtocolMessage *msgPtr, unsigned short len, char *buf, CWProtocolVendorSpecificValues *valPtr);      /**< 响应函数 */
} CWFreqTable;
