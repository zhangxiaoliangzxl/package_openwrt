...
extern int gAPFoundACType;
option43_cfg option43;
...
s32 kmod_communicate_init(char *devmac)
{
	...
	if( ret != CW_OK)
	{
		printf("kernel capwap module not start!! socket bind error %d %s\n", errno, strerror(errno));
		return CW_FAIL;
	}
	/* 构造目的端点，用于发送 */  
	...
	/*< 下发自己的pid，并在得到AC下发的版本ID时，告诉内核，集中转发时使用*/
...
char *getOption43Info()
{
	return (char *)option43.ipInfo;
}
...
static void dtt_parse_option43(u8 *buf){
	...
	memset(&option43, 0, sizeof(option43_cfg));
	memcpy(&option43, buf, sizeof(option43_cfg));
	printf("res1=%d, res2=%d\n", option43.res1, option43.res2);

	for(i = 0;i < 4;i ++){
		printf("%d: %d-%d-%d.%d.%d.%d\n", i, option43.ipInfo[i].id, option43.ipInfo[i].len, option43.ipInfo[i].ip[0], option43.ipInfo[i].ip[1], option43.ipInfo[i].ip[2], option43.ipInfo[i].ip[3]);
	}
...
s32 dtt_nl_recv_from_kmod(dcma_udp_skb_info_t *buff)
{
	...
	switch(buff->type){
		case CW_NLMSG_RECORD_USER_IP:
            CWLog("recv sta dhcp info from kmod");
			dtt_record_user_ip(buff->data);
			break;
		case CW_NLMSG_RECORD_DHCP_OPTION_43:
			CWLog("get option 43 data from kmod");
