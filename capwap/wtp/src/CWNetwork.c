...
void CWNetResetSockPort()
{
	gSockPort = 0;
}

/*
 * Init network for client.  flag为第几次发送discover请求标志
 */
CWBool CWNetworkInitSocketClient(CWSocket *sockPtr, CWNetworkLev4Address *addrPtr, int flag)
{	
	int yes = 1;
...
#else
{
    ...
    CWDTTLog("get_ip lan %s", buffer);
    
    memset(str, 0 ,sizeof(str));
	sscanf(buffer, "%d.%d.%d.%d", str, str+1,str+2,str+3);
	strValue[0] = str[0];strValue[1] = str[1];strValue[2] = str[2];strValue[3] = str[3];
