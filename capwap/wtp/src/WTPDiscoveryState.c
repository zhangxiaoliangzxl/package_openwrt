...
/*_________________________________________________________*/
/*  *******************___FUNCTIONS___*******************  */
/*< ZDC最新单隧道协议，需要发送两个discover请求，两次端口号不同*/
static char gTwoDiscoverflag = 0;

void CWWTPSetDiscoverCount()
{
	gTwoDiscoverflag = 0;
}
...
CWStateTransition CWWTPEnterDiscovery() {
	...
	CWLog("\n");	
	CWDTTLog("######### Discovery State #########");

	CWStopTimers();
	/* reset Discovery state */
	gCWDiscoveryCount = 0;
	CWNetworkCloseSocket(gWTPSocket);
	while(!CWErr(CWNetworkInitSocketClient(&gWTPSocket, NULL, gTwoDiscoverflag))) {
		CWDTTLog("InitSocketClient gWTPSocket failed!!!   continue....");
		sleep(1);
		continue;
	}
	...
	for(i = 0; i < gCWACCfg->gCWACCount; i++) 
		gCWACCfg->gCWACList[i].received = CW_FALSE;
...
//	sleep(CWRandomIntInRange(gCWDiscoveryInterval, gCWMaxDiscoveryInterval));
	...
	CW_REPEAT_FOREVER {
		...
		for(i = 0; i < gCWACCfg->gCWACCount; i++) {
			...
			if(!(gCWACCfg->gCWACList[i].received)) {
				/* ...send a Discovery Request */

				CWProtocolMessage *msgPtr = NULL;
				
				/* get sequence number (and increase it) */
				gCWACCfg->gCWACList[i].seqNum = CWGetSeqNum();
				
				if(!CWErr(CWAssembleDiscoveryRequest(&msgPtr,
								     gCWACCfg->gCWACList[i].seqNum))) {
					exit(1);
				}
				
                CW_CREATE_OBJECT_ERR(gACInfoPtr, CWACInfoValues, return CW_QUIT;);
				CWLog("### send discover to %s\t###", gCWACCfg->gCWACList[i].address);
				CWNetworkGetAddressForHost(gCWACCfg->gCWACList[i].address, 
							   &(gACInfoPtr->preferredAddress));
				
				CWErr(CWNetworkSendUnsafeUnconnected(gWTPSocket,
									 &(gACInfoPtr->preferredAddress),
									 (*msgPtr).msg,
									 (*msgPtr).offset)); 
				/* 
...
CWBool CWReceiveDiscoveryResponse() {
	...
	for(i = 0; i < gCWACCfg->gCWACCount; i++) {
		...
		if(gCWACCfg->gCWACList[i].seqNum == seqNum) {
		
			CWUseSockNtop(&addr,
				      CWLog("Discovery Response from:%s", str););
			/* we received response from this address */
			gCWACCfg->gCWACList[i].received = CW_TRUE;
...
CWBool CWAssembleDiscoveryRequest(CWProtocolMessage **messagesPtr, int seqNum) {
	...
	if((!(CWAssembleMsgElemDiscoveryType(&(msgElems[++k])))) ||
	   ...
	   (!CWAssembleMsgElemWTPVendorSpecificPayloadWTPMAC(&(msgElems[++k]),14)) ||
	   (!CWAssembleMsgElemWTPVendorSpecificPayloadUnkown(&(msgElems[++k]),14)) ||
	   (!CWAssembleMsgElemWTPVendorSpecificPayloadWTPVersion(&(msgElems[++k]),200))
