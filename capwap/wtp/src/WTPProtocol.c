...
/*< 根据最新协议，该字段直接包含了mac，version，以及另一张卡的mac，下面3个接口弃用*/
CWBool CWAssembleMsgElemWTPVendorSpecificPayload(CWProtocolMessage *msgPtr, int offset) {
	...
	len = 3*(VENDOR_ID_LENGTH+PL_HEADER_LENGTH) + offset;
	// create message
	CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, len, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
...
//		 *(unsigned char *)(mac+4),  *(unsigned char *)(mac+5));
	...
	CWProtocolStoreRawBytes(msgPtr, gWtpPublicInfo.ethMac, 6);
//	CW_FREE_OBJECT(mac);
...
//	CW_COPY_MEMORY(version + 64, "1.1.9(Test)10", 13);
	CW_COPY_MEMORY(version, gWtpPublicInfo.hwModel, strlen(gWtpPublicInfo.hwModel));
	CW_COPY_MEMORY(version + 64, gWtpPublicInfo.hideFwMode, strlen(gWtpPublicInfo.hideFwMode));
	...
	CWProtocolStoreRawBytes(msgPtr, gWtpPublicInfo.wlan1Mac, 6);
