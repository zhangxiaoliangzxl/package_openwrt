...
CWBool CWDTTAddElementToList(CWList *list, unsigned char type, char *strValue, unsigned short size)
{
	CWMsgElemWTPEvent *data = NULL;
//	char *tmp = NULL;

	if(NULL == list)	return CW_FALSE;
#if 0
	int i = 0;
	for(i = 0;i < size;i ++){
		if(CW_MSG_ELEMENT_WTP_EVENT_WTP_MAC == type)
			printf("%02x ", *((unsigned char *)(strValue+i)));
		else
			printf("%d ", *((strValue+i)));
	}
	printf("\n");
#endif
	data = (CWMsgElemWTPEvent *)calloc(1, sizeof(CWMsgElemWTPEvent)+size);
	if(!data)
		return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL);
	data->type = type;
	data->length = size;
	if(strValue)
		CW_COPY_MEMORY(data->value, strValue, size);

	CWAddElementToListTail(list, data);
