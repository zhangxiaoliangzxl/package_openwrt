...
CWBool CWAssembleWTPEventRequest(CWProtocolMessage **messagesPtr,
				 ...
				 int PMTU,
				 int seqNum,
				 CWList msgElemList, unsigned short msgElemType, int staCount, int radioIndex) {
...
CWBool CWAssembleConfigurationUpdateResponse(CWProtocolMessage **messagesPtr,
	...
	msgElemCount = 1+protoValues->getWtpVersion;
...
//	CWLog("#######getWtpVersion = %d\n", protoValues->getWtpVersion);
//	if(updateRequestType == CW_MSG_ELEMENT_VENDOR_SPEC_PAYLOAD_CW_TYPE)
	...
	/*Result Code only*/
	if (!(CWAssembleMsgElemResultCode(msgElems+protoValues->getWtpVersion,resultCode))) {
		CW_FREE_OBJECT(msgElems);
		return CW_FALSE;
	}
...
//	CWProtocolVendorSpecificValues* vendorPtr=(CWProtocolVendorSpecificValues *)protocolValuesPtr; 
	...
	if(1 == protoValues->restartSystem)
	{
...
//			system("/etc/init.d/WTP restart");
	...
	if(1 == protoValues->restartNetwork)
	{
		...
		if(gRadio == 1){
			...
			if(gConfigParamFirstFlag){
				...
				system("if [ `ifconfig | grep wlan0 | grep -v grep | wc -l` -lt 1 ];then "WIFI_RESTART";fi");
			}
		}
		else if(gRadio == 2){
			system("i=0;while [ `ps | grep mac80211.sh | grep -v grep |  wc -l` -gt 0 ] && [ $i -lt 8 ];do i=`expr $i + 1`;sleep 1;done");
			if(gConfigParamFirstFlag){
				...
				system("if [ `ifconfig | grep wlan1 | grep -v grep | wc -l` -lt 1 ];then "WIFI_RESTART";fi");
			}
		}
#endif
	}

    if(1 == protoValues->restartwifi)
	{
		system(UCI_COMMIT_WIRELESS);
		/*< 现在的逻辑改为，AP起来之后，可以有原配置的SSID，所以，直接启动wifi即可*/
		if(0 && gConfigParamFirstFlag){
            #if 0
			if(gRadio == 1)
				system("if [ `ifconfig | grep wlan0 | grep -v grep | wc -l` -ge 1 ];then " NETWORK_RELOAD ";else chmod a+x /usr/sbin/wpad && wifi;fi;fi");
			else if(gRadio == 2)
				system("if [ `ifconfig | grep wlan1 | grep -v grep | wc -l` -ge 1 ];then " NETWORK_RELOAD ";else chmod a+x /usr/sbin/wpad && wifi;fi;fi");
            #endif
            gConfigParamFirstFlag = 0;
		}
		else
        {
            ...
            #if 0
			if(gRadio == 1)
				system("if [ `ifconfig | grep wlan0 | grep -v grep | wc -l` -ge 1 ];then " NETWORK_RELOAD ";else "WIFI_RESTART";fi &");
			else if(gRadio == 2){
				system("if [ `ifconfig | grep wlan1 | grep -v grep | wc -l` -ge 1 ];then " NETWORK_RELOAD ";else "WIFI_RESTART";fi &");
			}
            #endif
			//system(WIFI_RESTART);
		}
    ...
    if(1 == protoValues->restartNetwork)
    {
        system("i=0;while [ `ps | grep -e network -e wifi | grep -v grep | wc -l` -gt 0 ] && [ $i -lt 30 ];do i=`expr $i + 1`;sleep 1;done");
		system("echo \"network reload\" > /dev/console");
        /* switch config reload */
        system(SWITCH_RELOAD);
        system(NETWORK_RELOAD);
    }
    else if (1 == protoValues->restartwifi)
    {
        system("i=0;while [ `ps | grep wifi | grep -v grep | wc -l` -gt 0 ] && [ $i -lt 30 ];do i=`expr $i + 1`;sleep 1;done");
		system("echo \"wifi reload\" > /dev/console");
        WIFI_RELOAD(cmd, (gRadio - 1));
    }
