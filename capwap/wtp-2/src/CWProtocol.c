...
//	CWLog("elem size if %d", size);
	// create message
	if(elemType == CW_MSG_WTP_EVENT_STA_INFO)
	{
		CW_CREATE_PROTOCOL_MESSAGE(*msgPtr, size + VENDOR_ID_LENGTH + ELEMENT_ID + ELEMENT_LENGTH + REPORT_TIME_LENGTH + STA_COUNT_LENGTH, 
			return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	}
	else
	{
	...
	if(elemType == CW_MSG_WTP_EVENT_STA_INFO)
	{
		CWProtocolStore16(msgPtr, size + REPORT_TIME_LENGTH + STA_COUNT_LENGTH);
	}
	else
	{
	...
	if(elemType == CW_MSG_WTP_EVENT_STA_INFO)
	{
		...
		CWProtocolStore32(msgPtr, staCount);
	}
#if 0
