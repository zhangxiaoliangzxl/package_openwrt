...
int LoadWltpHeader(void *data, uint32_t seq, uint16_t length, uint16_t fragment)
{
	...
	head->vapIndex = 0;
	head->pad1 = 0;
	head->configid = htons(gTemplateID);
	head->pad2 = htonl(0);
...
void CWWLTPKeepAliveExpired(CWTimerArg arg) {
	...
	/*< gCWAPCardCount为单双卡*/
	*((short *)&send_data[index]) = htons(14+16*gCWAPCardCount);
	index += 2;

	//ELV -- VlanID
	*((short *)&send_data[index]) = htons(0x0a01);
	index += 2;
	*((short *)&send_data[index]) = htons(16*gCWAPCardCount);
	index += 2;
	getAllVapVlanID(vapVlan);

	for(i = 0;i < MAX_VAP*gCWAPCardCount; i++){
...
//			index += 16;
	...
	*((short *)&send_data[index]) = htons(0x0a02);
	index += 2;
	*((short *)&send_data[index]) = htons(6);//+16*((gCWAPCardCount == 2)?2:3)
	index += 2;
	memcpy(&send_data[index],  g_DevMAC, 6);
	index += 6;
