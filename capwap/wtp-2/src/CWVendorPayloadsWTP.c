...
//	CWVendorWumValues *wumPtr;
	char *cmd = NULL;
	unsigned short vendorPayloadLen = 0;

	/*配置UCI的内存申请放外面，避免不断malloc*/
	CW_CREATE_STRING_ERR(cmd, UCI_CMD_LENGTH, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	
	CWParseMessageElementStart();

	while(msgPtr->offset <= len) {

		valPtr->vendorPayloadType = (unsigned short) CWProtocolRetrieve16(msgPtr);
		vendorPayloadLen = (unsigned short) CWProtocolRetrieve16(msgPtr);
		
		CWLog("parse vendor type ..., type is 0x%04x, len is %d\n", valPtr->vendorPayloadType, vendorPayloadLen);
		
		switch(valPtr->vendorPayloadType) 
        	{
			case CW_DTT_MSG_ELEMENT_GENETIC_GET_WTP_INFO:
				valPtr->getWtpVersion = 1;
				msgPtr->offset += vendorPayloadLen;
				break;
			case CW_DTT_MSG_ELEMENT_GENETIC_WTP_GET_FILE_FILENAME:
				DTTParseFirmwareFilename(msgPtr, vendorPayloadLen);
				break;
			case CW_DTT_MSG_ELEMENT_GENETIC_WTP_GET_FILE_AC_IPADDR:
				DTTParseFTPServerIP(msgPtr, vendorPayloadLen);
				break;
			case CW_DTT_MSG_ELEMENT_GENETIC_WTP_GET_FILE_FTP_USER:
				DTTParseFTPServerUser(msgPtr, vendorPayloadLen);
				break;
			case CW_DTT_MSG_ELEMENT_GENETIC_WTP_GET_FILE_FTP_PASSWD:
				DTTParseFTPServerPwd(msgPtr, vendorPayloadLen);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WTP_UPGRADE_FIRMWARE:
				DTTParseDoUpdate(cmd);
				break;
            /* wireless */
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_MODE:
				/*< 一个配置出错，不能影响其他的配置，所以，直接break即可*/
				DTTParseUCIWirelessHwmode(msgPtr, cmd, valPtr);
				break;                
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_IS_DISABLED:
				if(!DTTParseUCIWirelessIsDisable(msgPtr, cmd, valPtr))
				{
					//goto EXIT;
				}
				break;  
			...
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_COUNTRY_CODE:
				if(!DTTParseUCIWirelessCountryCode(msgPtr, cmd, valPtr))
				{
					//goto EXIT;
				}
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_CHANNEL:
				if(!DTTParseUCIWirelessChannel(msgPtr, cmd, valPtr))
				{
					//goto EXIT;
				}
				break;
//			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_DATA_RATE:
//				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_BEACON_INTERVAL:
				if(!DTTParseUCIWirelessBeaconInterval(msgPtr, cmd, valPtr))
				{
					//goto EXIT;
				}
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_RTS_LIMITE:
				if(!DTTParseUCIWirelessRTS(msgPtr, cmd, valPtr))
				{
					//goto EXIT;
				}
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_DTIM_INTERVAL:
				DTTParseUCIWirelessDtimInterval(msgPtr, cmd, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_TX_POWER:
				if(!DTTParseUCIWirelessTxPower(msgPtr, cmd, valPtr))
				{
					//goto EXIT;
				}
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_VAP_VLAN:
				if(!DTTParseUCIWirelessVAPVlan(msgPtr, cmd, valPtr))
				{
					//goto EXIT;
				}
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_VAP_SWITCH:
				if(!DTTParseUCIWirelessVAPSwitch(msgPtr, cmd, valPtr))
				{
					//goto EXIT;
				}
				break;
#endif                
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_SSID_NAME:
				if(!DTTParseUCIWirelessSSID(msgPtr, cmd, vendorPayloadLen, valPtr))
				{
					//goto EXIT;
				}
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_SSID_HIDE:
				if(!DTTParseUCIWirelessHideSSID(msgPtr, cmd, valPtr))
				{
					//goto EXIT;
				}
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_VAP_WPA_PSK:
				DTTParseUCIWirelessWPAPSK(msgPtr, cmd, vendorPayloadLen, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_VAP_AUTH_TYPE:
				DTTParseUCIWirelessAuthType(msgPtr, cmd, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_VAP_PASSWD_TYPE:
				DTTParseUCIWirelessEncryptType(msgPtr, cmd, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_ISOLATE_IS_DISABLED:
				if(!DTTParseIsolate(msgPtr, cmd, valPtr))
				{
					//goto EXIT;
				}
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WIRELESS_AUTO_CHANNEL:
				if(!DTTParseUCIWirelessAutoChannel(msgPtr, cmd, valPtr))
				{
					//goto EXIT;
				}
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_LEAD_FRAME_LENGTH:
				DTTParseUCIWirelessPreamble(msgPtr, cmd, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_RADIUS_SERVER_IPADDR:
				DTTParseUCIRadiusServer(msgPtr, cmd, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_RADIUS_SERVER_PORT:
				DTTParseUCIRadiusPort(msgPtr, cmd, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_RADIUS_SERVER_KEY:
				DTTParseUCIRadiusSecret(msgPtr, cmd, vendorPayloadLen, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WTP_PASSWD:
				if(!DTTParseUCIAPPassword(msgPtr, cmd, vendorPayloadLen, valPtr))
				{
					//goto EXIT;
				}
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WMM:
				DTTParseUCIWirelessWmm(msgPtr, cmd, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_VAP_MAX_STA_COUNT:
				DTTParseUCIVapMaxSta(msgPtr, cmd, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_LOCATION_TRANSMIT:
				if(!DTTParseLocalForwardSwitch(msgPtr, cmd, valPtr))
				{
					//goto EXIT;
				}
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_WTP_HOSTNAME:
				if(!DTTParseUCIAPName(msgPtr, cmd, vendorPayloadLen, valPtr))
				{
					//goto EXIT;
				}
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_BASE_SSID_BANDWIDTH_CONTROL:
				DTTParseVAPTrafficLimitSwitch(msgPtr, cmd, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_BASE_SSID_UP_BANDWIDTH_CONTROL:
				DTTParseVAPUploadLimit(msgPtr, cmd);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_BASE_SSID_DOWN_BANDWIDTH_CONTROL:
				DTTParseVAPDownloadLimit(msgPtr, cmd, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_11N_ONLY_CHANNEL_MODE:
				if(!DTTParseUCI11nChanMode(msgPtr, cmd, valPtr))
				{
					//goto EXIT;
				}
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_11N_ONLY_SHORT_GI_PROTECT:
				DTTParseUCIShortGI(msgPtr, cmd, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_AP_ASK_AC_AUTH_MACADDR_AC_RESPOSE:
				DTTParseACLControl(msgPtr, cmd, vendorPayloadLen, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_REBOOT_WTP:
				DTTParseRebootAP(msgPtr, cmd, valPtr);
				break;
			case CW_DTT_MSG_ELEMENT_CONFIG_RESET_FACTORY_SETTING:
				DTTParseResetFactoryAP(msgPtr, cmd, valPtr);
				break;
			/*< 模板ID，此变量在集中转发中比较重要，所以，会在检测到这个值改变的时候，发送给内核*/
			case CW_DTT_MSG_ELEMENT_CONFIG_SET_FORMWORK_NUM:
				DTTParseTemplateID(msgPtr);
				break;
//			case CW_DTT_MSG_ELEMENT_CONFIG_TAP_FLUX_CONTROL_STA_LIST:
				/*< 基于sta mac的上下行限速*/
//				DTTParseStaLimit(msgPtr, cmd, vendorPayloadLen, valPtr);
//				break;
			default:
				{
					CWLog("Unregistered type ..., type is %d, len is %d", valPtr->vendorPayloadType, vendorPayloadLen);
					CWLog("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++==");
                  msgPtr->offset += vendorPayloadLen;
					break;
...
EXIT:
	CWLog("Parse vendorPayload failed. tpye=0x%04x\n", valPtr->vendorPayloadType);
	CW_FREE_OBJECT(cmd);
	return CW_FALSE;
}
