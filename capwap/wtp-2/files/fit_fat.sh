...
bridge_set(){
	if [ "$1" -eq "1" ];then
	    ...
	    uci set network.@switch[-1].enable_vlan=0
	    uci commit network
	fi
...
if [ "$WTPENABLE" -eq "0" ] && [ "$APMODE" == "fit" ];then
    ...
    firewall_set 0
    bridge_set 0
    mv /etc/modules.d/99-cloud_wlan /usr/capwap/
    killall WTP
    crontab -r
...
else if [ "$WTPENABLE" -eq "1" ] && [ "$APMODE" == "fat" ];then
        echo "set Fit mode"
        dnsmasq_set 1
        firewall_set 1
        bridge_set 1
        echo "*/10 * * * * /usr/capwap/nmapip.sh > /tmp/capwap/ip-mac" > /etc/crontabs/root
        cp -rf /usr/capwap/99-cloud_wlan /etc/modules.d/99-cloud_wlan
