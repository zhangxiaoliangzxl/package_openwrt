...
static unsigned char switch_char(char chStr)
{
	if (chStr >= '0' && chStr <= '9')
	{
		return (chStr - '0');
...
void mac2hex(const char *macstr, unsigned char *machex)
{
	...
	snprintf(mac_scr, sizeof(mac_scr), "%s", macstr);
	char *pchStr = strtok(mac_scr, ":");

	while (NULL != pchStr)
	{
		machex[nTotal++] = (switch_char(*pchStr) << 4) | switch_char(*(pchStr + 1));
		pchStr = strtok(NULL, ":");
	}
}
