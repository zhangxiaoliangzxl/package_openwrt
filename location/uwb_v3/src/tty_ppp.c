...
int recevice_from_tty(int fd, Thread_data *ttyread_buffer)
{
	...
	while (TRUE)
	{
		...
		if (likely(ret > 0))
		{
			if (likely(event_tty.events & EPOLLIN))
			{
				...
				while (TRUE)
				{
					...
					if (likely(recv_len > 0))
					{
						...
						if (unlikely(0 == Rbuf_AddOne(ttyread_buffer->ring_buffer, read_data)))
						{
							fullcnt++;
						}
						else
...
static void parse_uwb_heart(ppp_uwb_data *uwb_data)
{
	...
	{
		log_w("data length is not match syshrt_type");
		return;
	}
	...
	if (uwb_data->type == DATATYPE_REPORT && uwb_data->port == REPORT_SYSHRT)
	{
		...
		log_d("(uwbheart)localid %d, runtime %u, payload %u", pdata->aid, pdata->runtime, pdata->payload);
...
{
	log_i("receive a uwb cfg ack !");
	if (uwb_data->length != sizeof(uwb_vercfg_type))
	{
		log_w("data length is not match uwb_vercfg_type !");
		return;
	}
	...
	log_i("(uwbcfg)-----------------------------");
	log_i("(uwbcfg)  matid %u", vercfg->mid);
	log_i("(uwbcfg)  localid %u", vercfg->aid);
	log_i("(uwbcfg)  panid %u", vercfg->panid);
	log_i("(uwbcfg)  channel %u", vercfg->channel);
	log_i("(uwbcfg)  coarsegain %u", vercfg->coarsegain);
	log_i("(uwbcfg)  finegain %.1f", ((float)vercfg->finegain * 1.0) / 10);
	log_i("(uwbcfg)  pcode %u", vercfg->preamble);
	log_i("(uwbcfg)  pa %d", cfg_ctrlbit->pa);
	log_i("(uwbcfg)  pwr %d", cfg_ctrlbit->pwr);
	log_i("(uwbcfg)  uwbversion %u", vercfg->version);
	log_i("(uwbcfg)-----------------------------");
...
{
	log_i("receive a uwb getrole ack !");
	if (uwb_data->length != sizeof(uwb_role_type))
	{
		log_w("data length is not match uwb_role_type !");
		return;
	}
	...
	switch (rolecfg->role)
	{
		...
		case ROLE_TYPE_S:
			...
			break;
		default:
			log_w("unknown role type, %04x", rolecfg->role);
			break;
	}
...
static void parse_normal_ack(ack_type *uwback)
{
	switch (uwback->ack_datatype)
	{
		case DATATYPE_CMD:
			...
			{
				log_i("(ack) receive uwb_cmd reset ack !");
			}
			else if (uwback->ack_dataport == CMD_getgain)
			{
				log_i("(ack) receive uwb_cmd getgain ack !");
			}
			else if (uwback->ack_dataport == CMD_getcomm)
			{
				log_i("(ack) receive uwb_cmd getcomm ack !");
			}
			break;
		case DATATYPE_CMD1:
			if (uwback->ack_dataport == CMD1_matid)
			{
				log_i("(ack) receive uwb_cmd matid ack !");
			}
			else if (uwback->ack_dataport == CMD1_localid)
			{
				log_i("(ack) receive uwb_cmd localid ack !");
			}
			else if (uwback->ack_dataport == CMD1_panid)
			{
				log_i("(ack) receive uwb_cmd panid ack !");
			}
			else if (uwback->ack_dataport == CMD1_pcode)
			{
				log_i("(ack) receive uwb_cmd pcode ack !");
			}
			else if (uwback->ack_dataport == CMD1_ch)
			{
				log_i("(ack) receive uwb_cmd channel ack !");
			}
			else if (uwback->ack_dataport == CMD1_mode)
			{
				log_i("(ack) receive uwb_cmd mode ack !");
			}
			else if (uwback->ack_dataport == CMD1_enpalna)
			{
				log_i("(ack) receive uwb_cmd enpalna ack !");
			}
			else if (uwback->ack_dataport == CMD1_sync)
			{
				log_i("(ack) receive uwb_cmd sync ack !");
			}
			else if (uwback->ack_dataport == CMD1_lpf)
			{
				log_i("(ack) receive uwb_cmd lpf ack !");
			}
			else if (uwback->ack_dataport == CMD1_coarsegain)
			{
				log_i("(ack) receive uwb_cmd coarsegain ack !");
			}
			else if (uwback->ack_dataport == CMD1_finegain)
			{
				log_i("(ack) receive uwb_cmd finegain ack !");
			}
			else if (uwback->ack_dataport == CMD1_syshrt)
			{
				log_i("(ack) receive uwb_cmd syshrt ack !");
			}
			else if (uwback->ack_dataport == CMD1_extpapower)
			{
				log_i("(ack) receive uwb_cmd extpapower ack !");
			}
			break;
		case DATATYPE_CMD2:
			if (uwback->ack_dataport == CMD2_mrang)
			{
				log_i("(ack) receive uwb_cmd mrang ack !");
			}
			break;
		case ZIGBEE_SET:
			if (uwback->ack_dataport == ZIGBEE_SET_net)
			{
				log_i("(ack) receive zigbee_cmd net ack !");
			}
			else if (uwback->ack_dataport == ZIGBEE_SET_pow)
			{
				log_i("(ack) receive zigbee_cmd pow ack !");
			}
			else if (uwback->ack_dataport == ZIGBEE_SET_tagfreq)
			{
				log_i("(ack) receive zigbee_cmd tagfreq ack !");
			}
			break;
		case DATATYPE_ACK:
			if (uwback->ack_dataport == ACK_GETROLE)
			{
				log_i("(ack) receive uwb_cmd setrole ack !");
			}
			break;
		default:
			log_w("unknown type %02x", uwback->ack_datatype);
			break;
	}
}
...
static void parse_uwb_normalack(ppp_uwb_data *uwb_data)
{
	...
	{
		log_w("data length is not match ack_type !");
		return;
	}

	ack_type *uwback = (ack_type *)(uwb_data->data);
	// log_i("receive a uwb normal ack, seq %u type %02x port %02x !", uwback->ack_sequence, uwback->ack_datatype,
	// uwback->ack_dataport);
...
static void parse_uwb_ack(ppp_uwb_data *uwb_data)
{
	switch (uwb_data->port)
	{
		...
		case ACK_GETROLE:
			...
			break;
		default:
			// log_i("unknown data, type %02x port %02x", uwb_data->type, uwb_data->port);
			uwb_data->needsend = 1;
			break;
	}
}
...
static void strdump_zigbee_strout(ppp_uwb_data *uwb_data)
{
	...
	{
		log_w("data length over MAXLENGTH_HEX_UWB!");
...
static int align_uwbdata(ppp_uwb_data *uwb_data, char *out, unsigned int *out_len)
{
	switch (uwb_data->uwbtype)
	{
		case Position:
		{
			...
			{
				log_w("data length %d not equal length of tdoa_type %d, drop data", uwb_data->length,
					  sizeof(tdoa_type));
				goto ERR;
			}
		...
		case Sync:
		{
			...
			{
				log_w("data length %d not equal length of sync_data %d , drop data", uwb_data->length,
					  sizeof(sync_type));
				goto ERR;
			}
		...
		case Status:
		{
			...
			{
				log_w("data length not equal length of zigbee_report_type, drop data");
				goto ERR;
			}
		...
		case Tof:
		{
			...
			{
				log_w("data length %d not equal length of tof_type %d, drop data", uwb_data->length, sizeof(tof_type));
				goto ERR;
			}
		...
		case Tdoag:
		{
			...
			{
				log_w("data length %d not equal length of tdoag_type %d, drop data", uwb_data->length,
					  sizeof(tdoag_type));
				goto ERR;
			}
		...
		case Tdoainfo:
		{
			...
			{
				log_w("data length %d not equal length of tdoainfo_type %d, drop data", uwb_data->length,
		...
		case Tdoawarn:
		{
			...
			{
				log_w("data length %d not equal length of tdoawarn_type %d, drop data", uwb_data->length,
		...
		case Barometer:
		{
			...
			{
				log_w("data length %d not equal length of barometer_type %d, drop data", uwb_data->length,
		...
		case Tdoasensor:
		{
			...
			{
				log_w("data length %d not equal length of tdoasensor_type %d, drop data", uwb_data->length,
...
+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
heade      1byte  //0x7E
timestamp  8byte  //uint64_t microseconds 1585105782761197
mac        6byte  //unsigned char mac16[6]
datalength 4byte  //uint32_t
datatype   1byte  //uint8_t
dataport   1byte  //uint8_t
data	   nbyte  //pointer
crc16      2byte  //uint16_t
end        1byte  //0x7E
*******************************************************************************/
...
static int tlv_encode(ppp_uwb_data *uwb_data, config *conf, char *out, unsigned int *out_len)
{
	...
	/* timestamp 8 byte */
	sec = uwb_data->frame_time.tv_sec;
	time = sec * 1000 * 1000 + uwb_data->frame_time.tv_usec; // microseconds
	memcpy(&tlv_buf[0], &time, sizeof(time));

	/* mac 6 byte */
	memcpy(&tlv_buf[8], conf->mac16, MAC_ADDRESS_LEN);
	...
	*/
	...
	memcpy(&tlv_buf[8 + 6 + 4 + 1 + 1], uwb_data->data, uwb_data->length);

	/* data length 4 byte */
	datalen = sizeof(uwb_data->type) + sizeof(uwb_data->port) + uwb_data->length;
	memcpy(&tlv_buf[8 + 6], &datalen, sizeof(datalen));

	/* crc16, 2 byte , timestamp + machex + datalength + data(type + port + uwb_data->data) */
...
/* func: parse uwbdata from ppp frame buffer and sendto send thread */
int full_num = 0;
ppp_uwb_data uwb_data_tmp;
ppp_uwb_data *uwb_data = &uwb_data_tmp;
char ppp_decode_buff[PPP_FRAME_LENGTH * 2] = {0};

static void uwb_parse_func(ppp_frame_data *frame_data, Thread_data *send_buffer)
{
	char *pNext = NULL;
	int len = 0;
	uint16_t crc = 0;
...
#endif
	/* stm32hexdebug */
	if (send_buffer->con->stm32hexdebug == 1)
	{
		memset(send_buffer->con->print_buff, 0, PRINT_BUFF_SIZE);
		hex_data(frame_data->ppp_frame, frame_data->ppp_frame_len, send_buffer->con->print_buff);
		printdata(send_buffer->con->print_buff);
	}
	...
	*******************************************************************************/
...
#ifdef DEBUG1
	printf("[pppframe]receive a ppp frame, seq:%d datalen:%d crc:%04x\n", ppp_data.sequence, ppp_data.datalength,
		   ppp_data.crc);
#endif
	/* crc check */
	crc = CRC_Check(DEF_CRC_INIT, (ppp_data.data - sizeof(uint8_t)), (ppp_data.datalength + sizeof(uint8_t)));
#ifdef DEBUG
	printf("[pppframe]crc start %02x length %d , src_crc %04x our_crc %04x \n", (ppp_data.data - sizeof(uint8_t))[0],
		   (ppp_data.datalength + sizeof(uint8_t)), ppp_data.crc, crc);
#endif

	if (unlikely(crc != ppp_data.crc))
	{
		log_e("[pppframe]seq %d frame crc error, src_crc %04x , our_crc %04x ,drop this frame!", ppp_data.sequence,
			  ppp_data.crc, crc);
#ifdef DEBUG4
		printf_hex("[pppframe]receive a ppp frame", frame_data->ppp_frame, frame_data->ppp_frame_len);
		printf_hex("[pppframe]ppp_decode and del heade && end flag", ppp_decode_buff, ppp_decode_len);
	...
	memset(uwb_data, 0, sizeof(ppp_uwb_data));
	...
	/*format data and tail to send ring_buffer */
	if (likely(send_buffer->con->socktype == SOCK_TCP))
	{
		if (likely(tcp_status == TCP_connected && uwb_data->needsend == 1))
		{
#ifdef TLV_SEND
			/* format to tlv */
#ifdef TLV_ENCODE
			if (tlv_uwbdata(uwb_data, send_buffer->con, &tlv_data) > 0)
			{
#ifndef USE_SEM
				pthread_mutex_lock(&(send_buffer->mutex));
#endif

				if (unlikely(0 == Rbuf_AddOne(send_buffer->ring_buffer, &tlv_data)))
				{
					full_num++;
				}
				else
				...
				sem_post(&send_buffer->sem);
#endif
			}
#else
			tlv_data.length = frame_data->ppp_frame_len;
			memset(tlv_data.data, 0, sizeof(tlv_data.data));
			memcpy(tlv_data.data, frame_data->ppp_frame, frame_data->ppp_frame_len);

			if (send_buffer->con->print_enable == 1)
			{
				memset(send_buffer->con->print_buff, 0, PRINT_BUFF_SIZE);
				hex_data(tlv_data.data, tlv_data.length, send_buffer->con->print_buff);
				printdata(send_buffer->con->print_buff);
			}
			...
			if (0 == Rbuf_AddOne(send_buffer->ring_buffer, &tlv_data))
			{
				full_num++;
			}
			else
...
#else
			/* format to json */
			pJson_data = json_uwbdata(uwb_data, send_buffer->con);
			if (NULL != pJson_data)
			{
				if (uwb_data->uwbtype != Other)
				{
					if ((uwb_data->uwbtype == Sync) && (0 == send_buffer->con->debug_enable))
					{
						/* do not sednd */
						// log_i("sync is disabled , don't send");
					}
					else
					{
#ifndef USE_SEM
						...
						if (0 == Rbuf_AddOne(send_buffer->ring_buffer, pJson_data))
						{
							full_num++;
						}
						else
			...
			if (unlikely(full_num >= 10))
			{
				full_num = 0;
				log_w("uwb buffer is full");
			}

#ifdef DEBUG
			printf("[pppframe] add one format data to send buffer\n");
#endif
		}
	}
	else if (send_buffer->con->socktype == SOCK_UDP)
	{
		/* format to tlv */
		if (unlikely(send_buffer->con->udp2can == 0))
		{
			if (tlv_uwbdata(uwb_data, send_buffer->con, &tlv_data) > 0)
			{
#ifndef USE_SEM
		...
		else /* encode for udp2can */
		{
			if ((uwb_data->type == 0xB4) && (uwb_data->port == 0xBA))
			{
				tlv_data.length = uwb_data->length;
				memset(tlv_data.data, 0, sizeof(tlv_data.data));
				memcpy(tlv_data.data, uwb_data->data, uwb_data->length);
...
void *ppp_frame_thread_func(void *indata)
{
	...
	while (TRUE)
	{
		...
		if (ret <= 0)
		{
...
#ifdef USE_SEM
			sem_wait(&(data->ttyread_buffer->sem));
#else
			...
			wait_ret = pthread_cond_timedwait(&(data->ttyread_buffer->cond), &(data->ttyread_buffer->mutex), &tv);
			if (wait_ret != 0)
			{
				if (locked)
				{
				...
				log_w("pppframe worker timedwait timeout !");
			}
#endif
		}
		else
		{
			...
			{
				log_w("read datalen over ppp_frame_length, not ppp data , drop");
...
#endif
							log_w("not find start PPP_FRAME_FLAG, drop this data!");
				...
				if (ppp_status == pppstatus_start && pStart != NULL)
				{
					...
					for (n = 1; n < datalen; n++) // pStart+1
					{
						if (pStart[n] == PPP_FRAME_FLAG)
						{
...
#endif
							/* parse to uwb data from ppp && tail to send buffer */

							if (likely(ppp_frame_len >= 8 && ppp_frame_len <= sizeof(ppp_frame_data)))
							{
								/* set timestap */
								...
								uwb_parse_func(frame_data, data->send_buffer);
							...
							{
								log_w("pppframe_len %d is error, drop", ppp_frame_len);
							}
