...
static int get_center_freq_vht(unsigned int freq, enum chan_width width)
{
	...
	switch(width) {
		...
		case CHAN_WIDTH_8080:
			...
			break;
		default:
			printlog("%s is not VHT\n", channel_width_string(width, -1));
	}
	return center1;
}
...
bool channel_change(int idx, enum chan_width width, bool ht40plus)
{
	...
	if(conf.fchans[channels.chan[idx].chan] != 0xff)
	{
		return false;
	}
	...
	{
		printlog("ccerter1 == 0 && CHAN_WIDTH_20_NOHT\n");
		return false;
	}
...
bool channel_auto_change(void)
{
	...
	{
		conf.channel_idx = channel_find_index_from_chan(1);
		return false;
	}
	...
	{
		if(the_time.tv_sec - the_time1.tv_sec > 2592000)
		{
			return 126;
		}
		return 127;
...
bool channel_init(void)
{
	...
	if (conf.channel_set_num > 0) {
		...
		printlog("Setting configured channel %d, ini_idx %d\n", conf.channel_set_num, ini_idx);
