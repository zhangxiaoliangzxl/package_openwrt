				...
				const char *const interface)
{
	...
	if (interface) { //TODO: PHY commands don't need interface name but wiphy index
		...
		//int if_index = phy_lookup(interface);
		if (!if_index) {
		//if (if_index < 0) {
			fprintf(stderr, "phy interface %s does not exist\n", interface);
			goto nla_put_failure;
		}
...
bool ifctrl_iwget_phy_info(const char *const interface)
{
	struct nl_msg *msg;
	bool ret;
...
bool ifctrl_iwget_interface_check(const char *const interface)
{
	struct nl_msg *msg;
	bool ret;
	...
	ret = nl80211_send_recv(sock, msg, NULL, NULL); /* frees msg */
	return ret;
}
