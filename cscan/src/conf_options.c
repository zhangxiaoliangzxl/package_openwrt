...
static bool conf_server_addr(const char* value) {
	snprintf(conf.sip, IP_STR_LEN, "%s", value);
	return true;
}

static bool conf_server_port(const char* value) {
	conf.sport = atoi(value);
	return true;
}

static bool conf_upmode(const char* value) {
	conf.upmode = (UPMODE)atoi(value);
	return true;
}

static bool conf_url(const char* value) {
	snprintf(conf.surl, MAX_URL_LEN, "%s", value);
	return true;
}

static bool conf_apID(const char* value) {
	snprintf(conf.apID, MAC_STR_LEN, "%s", value);
	return true;
}
...
static bool conf_filter_mac(const char* value) {
	static int n;
	if (n >= MAX_FILTERMAC) {
		printlog("Can only handle %d MAC filters\n", MAX_FILTERMAC);
		return false;
	}
...
static bool conf_filter_channel(const char* value) {
	static unsigned char n = 0;
	if (n >= MAX_FILTERCHAN) {
		printf("Can only handle %d channel filters", MAX_FILTERCHAN);
		return false;
	}

	if(strcmp(value, "default") == 0)
	{
		memset(conf.fchans, 0x00, sizeof(conf.fchans));
		conf.fchans[1] = 0xff;
		conf.fchans[6] = 0xff;
		conf.fchans[11] = 0xff;
		conf.fchans[149] = 0xff;
		conf.fchans[153] = 0xff;
		conf.fchans[157] = 0xff;
		conf.filterchann = 6;
	}
	else
	{
		if(n >= MAX_FILTERCHAN)
		{
			printlog("max channels reached!\n");
			return false;
		}

		if(n == 0 && conf.filterchann == 6)
		{
			memset(conf.fchans, 0x00, sizeof(conf.fchans));
		}

		conf.fchans[atoi(value)] = 0xff;
		n++;
		conf.filterchann = n;
	}
...
//#if 1
	...
	//设置全信道扫描信道,默认为1，6，11
	{  0 , "channels",		1, "default", 	conf_filter_channel},	
	//模式为:UDP/TCP时服务器地址
	{  0 , "server_addr",		1, "127.0.0.1", 	conf_server_addr},
	//模式为:UDP/TCP时端口号
	{  0 , "server_port",		1, "8119", 	conf_server_port},
	//上传模式:UDP,TCP,HTTP,HTTPS
	{  0 , "upmode",			1, "1", 	conf_upmode},
	//模式为:HTTP/HTTPS的URL地址
	{  0 , "server_url",		1, "http://www.airocov.com", 	conf_url},
	//设备UUID，唯一标识符br-lan MAC代替
	{  0 , "apID",				1, "shi:zhai", 	conf_apID},
...
bool config_handle_option(int c, const char* name, const char* value)
{
	...
	if (name != NULL)
		printlog("Ignoring unknown config option '%s' = '%s'\n", name, value);
	return false;
}
...
static void config_read_file(const char* filename)
{
	...
	if ((fp = fopen(filename, "r")) == NULL) {
		printlog("Could not open config file '%s'\n", filename);
		return;
	}
